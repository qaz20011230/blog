import {
  elizaInitials,
  elizaFinals,
  elizaQuits,
  elizaPres,
  elizaPosts,
  elizaSynons,
  elizaKeywords,
  elizaPostTransforms,
  ElizaKeyword
} from './elizaData';

export class ElizaBot {
  noRandom: boolean;
  capitalizeFirstLetter: boolean = true;
  debug: boolean = false;
  memSize: number = 20;
  version: string = "1.1 (TypeScript port)";
  
  quit: boolean = false;
  mem: string[] = [];
  lastchoice: number[][] = [];
  
  // Internal parsed data
  private _keywords: any[] = [];
  private _pres: Record<string, string> = {};
  private _posts: Record<string, string> = {};
  private _preExp: RegExp = /####/;
  private _postExp: RegExp = /####/;
  private _sentence: string = '';

  constructor(noRandomFlag: boolean = false) {
    this.noRandom = noRandomFlag;
    this._init();
    this.reset();
  }

  reset() {
    this.quit = false;
    this.mem = [];
    this.lastchoice = [];
    for (let k = 0; k < this._keywords.length; k++) {
      this.lastchoice[k] = [];
      const rules = this._keywords[k][2];
      for (let i = 0; i < rules.length; i++) {
        this.lastchoice[k][i] = -1;
      }
    }
  }

  private _init() {
    // Clone keywords to avoid mutating the original import if we re-instantiate
    // We need a deep clone because we modify the rules inside
    const keywords = JSON.parse(JSON.stringify(elizaKeywords));

    // parse data and convert it from canonical form to internal use
    // produce synonym list
    const synPatterns: Record<string, string> = {};
    for (const i in elizaSynons) {
      synPatterns[i] = '(' + i + '|' + elizaSynons[i].join('|') + ')';
    }

    // 1st convert rules to regexps
    // expand synonyms and insert asterisk expressions for backtracking
    const sre = /@(\S+)/;
    const are = /(\S)\s*\*\s*(\S)/;
    const are1 = /^\s*\*\s*(\S)/;
    const are2 = /(\S)\s*\*\s*$/;
    const are3 = /^\s*\*\s*$/;
    const wsre = /\s+/g;

    for (let k = 0; k < keywords.length; k++) {
      const rules = keywords[k][2];
      keywords[k][3] = k; // save original index for sorting
      
      for (let i = 0; i < rules.length; i++) {
        const r = rules[i];
        // check mem flag and store it as decomp's element 2
        if (r[0].charAt(0) === '$') {
          let ofs = 1;
          while (r[0].charAt(ofs) === ' ') ofs++;
          r[0] = r[0].substring(ofs);
          r[2] = true;
        } else {
          r[2] = false;
        }

        // expand synonyms
        let m = sre.exec(r[0]);
        while (m) {
          const sp = (synPatterns[m[1]]) ? synPatterns[m[1]] : m[1];
          r[0] = r[0].substring(0, m.index) + sp + r[0].substring(m.index + m[0].length);
          m = sre.exec(r[0]);
        }

        // expand asterisk expressions
        if (are3.test(r[0])) {
          r[0] = '\\s*(.*)\\s*';
        } else {
          m = are.exec(r[0]);
          if (m) {
            let lp = '';
            let rp = r[0];
            while (m) {
              lp += rp.substring(0, m.index + 1);
              if (m[1] !== ')') lp += '\\b';
              lp += '\\s*(.*)\\s*';
              if ((m[2] !== '(') && (m[2] !== '\\')) lp += '\\b';
              lp += m[2];
              rp = rp.substring(m.index + m[0].length);
              m = are.exec(rp);
            }
            r[0] = lp + rp;
          }
          
          m = are1.exec(r[0]);
          if (m) {
            let lp = '\\s*(.*)\\s*';
            if ((m[1] !== ')') && (m[1] !== '\\')) lp += '\\b';
            r[0] = lp + r[0].substring(m.index - 1 + m[0].length);
          }
          
          m = are2.exec(r[0]);
          if (m) {
            let lp = r[0].substring(0, m.index + 1);
            if (m[1] !== '(') lp += '\\b';
            r[0] = lp + '\\s*(.*)\\s*';
          }
        }
        
        // expand white space
        r[0] = r[0].replace(wsre, '\\s+');
        wsre.lastIndex = 0;
      }
    }

    // sort keywords by rank (highest first)
    keywords.sort(this._sortKeywords);
    this._keywords = keywords;

    // compose regexps and refs for pres and posts
    if (elizaPres.length) {
      const a: string[] = [];
      for (let i = 0; i < elizaPres.length; i += 2) {
        a.push(elizaPres[i]);
        this._pres[elizaPres[i]] = elizaPres[i + 1];
      }
      this._preExp = new RegExp('\\b(' + a.join('|') + ')\\b');
    }

    if (elizaPosts.length) {
      const a: string[] = [];
      for (let i = 0; i < elizaPosts.length; i += 2) {
        a.push(elizaPosts[i]);
        this._posts[elizaPosts[i]] = elizaPosts[i + 1];
      }
      this._postExp = new RegExp('\\b(' + a.join('|') + ')\\b');
    }
  }

  private _sortKeywords(a: any, b: any) {
    // sort by rank
    if (a[1] > b[1]) return -1;
    else if (a[1] < b[1]) return 1;
    // or original index
    else if (a[3] > b[3]) return 1;
    else if (a[3] < b[3]) return -1;
    else return 0;
  }

  transform(text: string): string {
    let rpl = '';
    this.quit = false;
    
    // unify text string
    text = text.toLowerCase();
    text = text.replace(/@#\$%\^&\*\(\)_\+=~`\{\[\}\]\|:;<>\/\\\t/g, ' ');
    text = text.replace(/\s+-+\s+/g, '.');
    text = text.replace(/\s*[,\.\?!;]+\s*/g, '.');
    text = text.replace(/\s*\bbut\b\s*/g, '.');
    text = text.replace(/\s{2,}/g, ' ');
    
    // split text in part sentences and loop through them
    const parts = text.split('.');
    for (let i = 0; i < parts.length; i++) {
      let part = parts[i];
      if (part !== '') {
        // check for quit expression
        for (let q = 0; q < elizaQuits.length; q++) {
          if (elizaQuits[q] === part) {
            this.quit = true;
            return this.getFinal();
          }
        }
        
        // preprocess
        let m = this._preExp.exec(part);
        if (m) {
          let lp = '';
          let rp = part;
          while (m) {
            lp += rp.substring(0, m.index) + this._pres[m[1]];
            rp = rp.substring(m.index + m[0].length);
            m = this._preExp.exec(rp);
          }
          part = lp + rp;
        }
        
        this._sentence = part;
        
        // loop through keywords
        for (let k = 0; k < this._keywords.length; k++) {
          const key = this._keywords[k][0];
          if (part.search(new RegExp('\\b' + key + '\\b', 'i')) >= 0) {
            rpl = this._execRule(k);
          }
          if (rpl !== '') return rpl;
        }
      }
    }
    
    // nothing matched try mem
    rpl = this._memGet();
    
    // if nothing in mem, so try xnone
    if (rpl === '') {
      this._sentence = ' ';
      const k = this._getRuleIndexByKey('xnone');
      if (k >= 0) rpl = this._execRule(k);
    }
    
    // return reply or default string
    return (rpl !== '') ? rpl : 'I am at a loss for words.';
  }

  private _execRule(k: number): string {
    const rule = this._keywords[k];
    const decomps = rule[2];
    const paramre = /\(([0-9]+)\)/;
    
    for (let i = 0; i < decomps.length; i++) {
      const m = this._sentence.match(decomps[i][0]);
      if (m != null) {
        const reasmbs = decomps[i][1];
        const memflag = decomps[i][2];
        let ri = (this.noRandom) ? 0 : Math.floor(Math.random() * reasmbs.length);
        
        if (((this.noRandom) && (this.lastchoice[k][i] > ri)) || (this.lastchoice[k][i] === ri)) {
          ri = ++this.lastchoice[k][i];
          if (ri >= reasmbs.length) {
            ri = 0;
            this.lastchoice[k][i] = -1;
          }
        } else {
          this.lastchoice[k][i] = ri;
        }
        
        let rpl = reasmbs[ri];
        if (this.debug) {
          console.log('match:\nkey: ' + this._keywords[k][0] +
            '\nrank: ' + this._keywords[k][1] +
            '\ndecomp: ' + decomps[i][0] +
            '\nreasmb: ' + rpl +
            '\nmemflag: ' + memflag);
        }
        
        if (rpl.search('^goto ', 'i') === 0) {
          const ki = this._getRuleIndexByKey(rpl.substring(5));
          if (ki >= 0) return this._execRule(ki);
        }
        
        // substitute positional params
        let m1 = paramre.exec(rpl);
        if (m1) {
          let lp = '';
          let rp = rpl;
          while (m1) {
            let param = m[parseInt(m1[1])];
            // postprocess param
            let m2 = this._postExp.exec(param);
            if (m2) {
              let lp2 = '';
              let rp2 = param;
              while (m2) {
                lp2 += rp2.substring(0, m2.index) + this._posts[m2[1]];
                rp2 = rp2.substring(m2.index + m2[0].length);
                m2 = this._postExp.exec(rp2);
              }
              param = lp2 + rp2;
            }
            lp += rp.substring(0, m1.index) + param;
            rp = rp.substring(m1.index + m1[0].length);
            m1 = paramre.exec(rp);
          }
          rpl = lp + rp;
        }
        
        rpl = this._postTransform(rpl);
        if (memflag) {
          this._memSave(rpl);
        } else {
          return rpl;
        }
      }
    }
    return '';
  }

  private _postTransform(s: string): string {
    // final cleanings
    s = s.replace(/\s{2,}/g, ' ');
    s = s.replace(/\s+\./g, '.');
    
    if (elizaPostTransforms.length) {
      for (let i = 0; i < elizaPostTransforms.length; i += 2) {
        const pattern = elizaPostTransforms[i];
        const replacement = elizaPostTransforms[i + 1] as string;
        s = s.replace(pattern, replacement);
        // If pattern is RegExp with global flag, reset lastIndex (though replace usually handles it)
        if (pattern instanceof RegExp) {
           pattern.lastIndex = 0;
        }
      }
    }
    
    // capitalize first char
    if (this.capitalizeFirstLetter) {
      const re = /^([a-z])/;
      const m = re.exec(s);
      if (m) s = m[0].toUpperCase() + s.substring(1);
    }
    return s;
  }

  private _getRuleIndexByKey(key: string): number {
    for (let k = 0; k < this._keywords.length; k++) {
      if (this._keywords[k][0] === key) return k;
    }
    return -1;
  }

  private _memSave(t: string) {
    this.mem.push(t);
    if (this.mem.length > this.memSize) this.mem.shift();
  }

  private _memGet(): string {
    if (this.mem.length) {
      if (this.noRandom) {
        const val = this.mem.shift();
        return val !== undefined ? val : '';
      } else {
        const n = Math.floor(Math.random() * this.mem.length);
        const rpl = this.mem[n];
        for (let i = n + 1; i < this.mem.length; i++) {
          this.mem[i - 1] = this.mem[i];
        }
        this.mem.length--;
        return rpl;
      }
    } else {
      return '';
    }
  }

  getFinal(): string {
    return elizaFinals[Math.floor(Math.random() * elizaFinals.length)];
  }

  getInitial(): string {
    return elizaInitials[Math.floor(Math.random() * elizaInitials.length)];
  }
}
