...
const SECTION_CLASS = "border-t border-gray-800/50 pt-10 pb-4";
const HEADING_CLASS = "text-sm font-mono text-primary tracking-[0.2em] uppercase mb-6";

function autoExpertise(posts: ReturnType<typeof getAllPosts>, locale: 'zh' | 'en') {
  ...
  const topTags = Object.entries(tagCount)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 12);
  return { topCats, topTags, totalPosts: posts.length, totalTags: Object.keys(tagCount).length, totalCats: Object.keys(catCount).length };
}

export default function About() {
  const { locale, t } = useLanguage();
  const posts = getAllPosts(locale);
  const { topCats, topTags, totalPosts, totalTags, totalCats } = autoExpertise(posts, locale);
  const recentPosts = posts.slice(0, 6);

  return (
    <div className="max-w-3xl mx-auto px-4 pb-16 page-enter">
      <Helmet>
          ...
          {UI.about.education.items.map((item, i) => (
            <div key={i} className="flex items-start gap-4 pl-2 border-l-2 border-gray-800">
              <div className="flex-1">
                ...
                {item.period.zh && <p className="text-gray-600 text-xs mt-0.5">{t(item.period.zh, item.period.en)}</p>}
              </div>
            </div>
          ))}
        </div>
      ...
      <div className={SECTION_CLASS}>
        ...
        <Link
          to="/"
          className="inline-flex items-center gap-1 mt-4 text-xs font-mono text-primary/60 hover:text-primary transition-colors"
        >
          {t(UI.about.writings.viewAll.zh, UI.about.writings.viewAll.en)}
        </Link>
      </div>
      ...
      <div className={SECTION_CLASS}>
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="text-center p-4 rounded-lg bg-gray-800/20 border border-gray-800/50">
            <div className="text-2xl font-bold text-gray-200 font-mono">{totalPosts}</div>
            <div className="text-xs text-gray-500 mt-1">{t(UI.about.stats.posts.zh, UI.about.stats.posts.en)}</div>
          </div>
          <div className="text-center p-4 rounded-lg bg-gray-800/20 border border-gray-800/50">
            <div className="text-2xl font-bold text-gray-200 font-mono">{totalCats}</div>
            <div className="text-xs text-gray-500 mt-1">{t(UI.about.stats.categories.zh, UI.about.stats.categories.en)}</div>
          </div>
          <div className="text-center p-4 rounded-lg bg-gray-800/20 border border-gray-800/50">
            <div className="text-2xl font-bold text-gray-200 font-mono">{totalTags}</div>
      ...
      {/* Contact */}
      <div className={`${SECTION_CLASS} pb-8`}>
        <h2 className={HEADING_CLASS}>
