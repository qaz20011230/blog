...
\title{UniRec：面向大规模推荐的序列建模与特征交互统一架构}
\author{Anonymous Author \thanks{University Name} \\ \texttt{email@example.com}
	\and
		黄思沛 \thanks{广东工业大学法学院}}
\date{\today}
...
\textbf{关键词}：推荐系统；统一架构；序列建模；特征交互；Transformer；信息论。
\end{abstract}
...
过去二十年，推荐研究沿着两条主线发展。一条主线聚焦于特征交互：对用于排序和校准的高维多域类别与上下文特征进行建模。另一条主线聚焦于序列行为：通过循环架构、注意力机制以及受自注意力启发的 Transformer 风格排序模型对时间有序的用户交互进行建模 \cite{vaswani2017attention}。两条传统均取得了显著成功。然而，它们在很大程度上是并行演化而非协同演化的。
...
\begin{itemize}
\item 识别并形式化了分离式推荐流水线背后的三个隐含假设，并说明为何它们在规模化时变得日益脆弱。
\item 提出 UniRec，一个以“先统一，后压缩”为核心组织原则的统一架构。
\item 提供了一个紧凑的结构性理由：其中源感知令牌化可赋予范畴论解读，而混合掩码与延迟压缩则可赋予几何解读，作为优化中的归纳约束。
\item 通过基于 GRU 的用户状态实例化持续更新，并定义替代距离视角以区分动态用户建模与冻结用户建模。
\item 针对挑战目标指定了公平且可证伪的经验协议，使得后续工程结果能够被结构性地解读，而非仅仅数值性地比较。
\end{itemize}
...
大量工业推荐模型遵循“编码再交互”逻辑，即便通过多个子系统实现。Deep Crossing \cite{shan2016deep}、Wide\&Deep \cite{cheng2016wide}、DeepFM \cite{guo2017deepfm}、DCN \cite{wang2017deep} 与 DCNv2 \cite{wang2021dcnv2} 代表了多域特征交互与排序的重要方法。这些方法在稀疏和上下文特征上极为有效，但它们通常将行为历史视为浅层聚合或外部产生的嵌入。

\subsection{序列建模}

另一条研究线关注用户行为的时间结构。GRU4Rec \cite{hidasi2016session}、DIN \cite{zhou2018deep}、DIEN \cite{zhou2019deep}、SASRec \cite{kang2018self} 与 BERT4Rec \cite{sun2019bert4rec} 使用循环或注意力机制建模序列依赖，而更广泛的 Transformer 范式 \cite{vaswani2017attention} 已更普遍地重塑了排序架构。然而在许多工业部署中，所得的序列表示在融入特征侧堆栈之前仍然被缩减为少量向量。

\subsection{特征交互架构}

除了 FM 类模型，xDeepFM \cite{lian2018xdeepfm} 与 AutoInt \cite{song2019autoint} 通过显式交叉层与自注意力交互丰富了跨特征表达能力。然而这些模型通常假设序列信息已被汇总。其后果是，事件级的序列-特征条件化要么缺失，要么极为有限。
...
其中
\begin{itemize}
\item \(S = (s_1,\ldots,s_n)\) 是有序历史事件序列；
\item \(\mathcal{F} = \{f_1,\ldots,f_m\}\) 是无序的域式类别特征集合；
\item \(\mathcal{C} = \{c_1,\ldots,c_q\}\) 是带有度量结构的连续标量或低维向量集合。
\end{itemize}
目标变量是响应 \(R\)，例如点击、转化或后点击转化。

\textbf{记号}。全文用 \(n\) 表示序列长度，\(m\) 表示类别特征域个数，\(q\) 表示连续上下文变量个数。

\subsection{三个隐含假设}

占主导地位的分离式流水线由以下假设支撑。

\begin{assumption}[早期压缩是充分的]
存在低维摘要 \(h_S = g(S)\)，使得在跨源交互之前用 \(h_S\) 替换 \(S\) 对联合 \(\mathcal{F},\mathcal{C}\) 预测 \(R\) 的任务相关信息损失可忽略。
\end{assumption}

\begin{assumption}[交互可以推迟]
大部分序列事件与域式特征之间的有用交互无需在序列压缩之前发生。
\end{assumption}

\begin{assumption}[分支可以准独立优化]
...
\subsection{为何早期压缩在结构上代价高昂}

设 \(H_t\) 表示 \(t\) 时刻可用的历史，\(X_t\) 表示预测时可用的非序列特征。一个分离系统通常计算 \(Z_t = g(H_t)\) 并从 \((Z_t, X_t)\) 预测 \(R\)。

\begin{proposition}[条件数据处理不等式]
对任意确定性压缩器 \(g\)，
\[
I(g(H_t);R\mid X_t) \leq I(H_t;R\mid X_t).
\]
等价地，
\[
I(g(H_t),X_t;R) \leq I(H_t,X_t;R).
\]
\end{proposition}
\begin{proof}
...
\begin{proposition}[早期压缩下的贝叶斯误差下界]
设 \(R\) 取值于有限标签集 \(\mathcal{R}\)。记 \(P_e(g)\) 为限制在 \((g(H_t),X_t)\) 上的任意预测器的贝叶斯误差。则
\[
P_e(g) \ge \frac{H(R\mid g(H_t),X_t)-1}{\log|\mathcal{R}|}
     = \frac{H(R\mid X_t) - I(g(H_t);R\mid X_t) - 1}{\log|\mathcal{R}|}.
\]
\end{proposition}
\begin{proof}
...
\textbf{序列令牌}。对于第 \(i\) 个事件 \(s_i\)，定义
\[
\Phi_S(s_i) = \mathrm{Embed}(item_i) + \mathrm{Embed}(action_i) + \mathrm{Pool}(content_i) + \mathrm{TimeEncoding}(t_i). \tag{4.1}
\]
时间编码需在弱但操作上有意义的层面上保持先后顺序，即事件间的序关系在嵌入空间中可区分。

\textbf{特征令牌}。对于域-值对 \(f_j = (\mathrm{field}_j, \mathrm{value}_j)\)，定义
\[
\Phi_F(f_j) = \mathrm{FieldEmbed}(\mathrm{field}_j, \mathrm{value}_j). \tag{4.2}
\]
由于特征集是无序的，对该子集的下游操作应当关于列表顺序具有置换不变性或置换等变性。

\textbf{上下文令牌}。对于连续标量或向量 \(c \in \mathcal{C}\)，定义
\[
\Phi_C(c) = \sigma(W_c c + b_c), \tag{4.3}
\]
其中 \(\sigma\) 为 Lipschitz 连续的非线性函数，例如 GELU 或 SiLU。

\textbf{统一令牌顺序}。令牌序列排列为
\[
...
\[
\mathbf{Z}^{(l+1)} = \mathrm{Softmax}\left(\frac{(\mathbf{Z}^{(l)}\mathbf{W}_Q)(\mathbf{Z}^{(l)}\mathbf{W}_K)^\top}{\sqrt{d_h}} + \mathbf{M}\right)\mathbf{Z}^{(l)}\mathbf{W}_V. \tag{4.4}
\]
掩码为
\[
...
其四个区块编码了源感知的可见性：
\begin{itemize}
\item \(\mathbf{M}_{SS}\) 是因果的，故序列令牌 \(i\) 只能关注 \(j \le i\) 的位置；
\item \(\mathbf{M}_{FF}\) 完全开放，故特征与上下文令牌密集连接；
\item \(\mathbf{M}_{FS}\) 完全开放，故特征侧令牌可以读取整个序列；
\item \(\mathbf{M}_{SF}\) 选择性开放，使得序列令牌可以访问上下文安全或用户侧信息，而不引入目标泄露。图 2 展示了区块结构。
\end{itemize}
...
并定义
\[
\mathrm{Compress}(\mathbf{S}) = \mathrm{Softmax}\left(\frac{Q_c \mathbf{S}^\top}{\sqrt{d}}\right)\mathbf{S}. \tag{4.6}
\]
...
\begin{proposition}[令牌化的函子性解读]
在 4.2 节的令牌化规则下，UniRec 在统一交互之前保持了顺序敏感的序列结构与顺序不敏感的特征结构之间的区分。
\end{proposition}

\textbf{设计直觉}。在序列侧，顺序通过时间编码显式表示。在特征侧，内在顺序的缺失通过域感知嵌入以及对待域顺序的置换不敏感性来尊重。这里的要点并非声称一个深刻的范畴论定理，而是说明为何源感知令牌化比简单拼接更具原则性。
...
\begin{theorem}[近单射统一表示界]
假设统一映射 \(\Phi\) 在 \((H_t,X_t)\) 的支撑集上是近单射的，即
\[
H(H_t,X_t \mid \Phi(H_t \oplus X_t)) \le \epsilon.
\]
则
\[
I(\Phi(H_t \oplus X_t); R) \ge I(H_t,X_t; R) - \epsilon. \tag{6.1}
\]
\end{theorem}
\begin{proof}
由于 \(\Phi(H_t \oplus X_t)\) 是 \((H_t,X_t)\) 的确定性函数，有
\[
...
\begin{corollary}[统一表示在 \(\epsilon\) 内支配分离压缩]
对任意确定性压缩器 \(g,h\)，
\[
I(\Phi(H_t \oplus X_t); R) \ge I(g(H_t),h(X_t); R) - \epsilon. \tag{6.2}
\]
\end{corollary}
\begin{proof}
由命题 3.4，联合确定性压缩不会增加任务相关信息：
\[
I(g(H_t),h(X_t); R) \le I(H_t,X_t; R).
\]
结合定理 6.1 即得结果。
\end{proof}

\textbf{解释}。定理 6.1 与推论 6.2 并非说任何名义上的统一模型都自动优于任何分离模型。它们表达了一个更基础的观点：如果一个统一表示近似保持联合输入结构，则其与响应的可用互信息在 \(\epsilon\) 碰撞项内不低于任何确定性压缩分离表示的互信息。因此，早期压缩默认是有损的；延迟压缩只有在充分的联合条件化之后才变得可辩护。

\begin{definition}[替代距离]
设 \(f_{\mathrm{dyn}}\) 是一个动态预测器，它随时间更新内部用户状态，\(\mathcal{H}_{\mathrm{stat}}\) 是一类静态预测器。对于时间范围 \(T\) 与损失 \(\ell\)，定义
\[
D_T(f_{\mathrm{dyn}}, \mathcal{H}_{\mathrm{stat}}) = \inf_{h \in \mathcal{H}_{\mathrm{stat}}} \mathbb{E}\left[ \frac{1}{T} \sum_{t=1}^{T} \ell\bigl(f_{\mathrm{dyn}}(x_{\le t}), h(x_{\le t})\bigr) \right]. \tag{6.3}
\]
该量并非为了闭式计算，而是为未来工程实验提供一个清晰目标：在匹配参数预算下近似最佳静态替代，并测量其在交互范围内模仿轨迹依赖模型的程度。
\end{definition}
...
\toprule
\textbf{研究} & \textbf{假设/读出的结论} & \textbf{状态} \\
\midrule
...
\section{挑战对齐、经验协议与工程议程}

本文档有意作为理论奠基技术报告。其目的是将架构假设、形式化保证与评估标准陈述得足够清晰，使后续工程研究能够对其进行证伪。

\textbf{挑战设定}。预期下游评估环境是“面向大规模推荐的序列建模与特征交互统一化”挑战。主要预测目标是 CVR，提交按单一 ROC-AUC 指标排名。除排行榜位置外，研讨会也明确重视统一模块创新与扩展规律分析。本报告的理论优先取向正是为了契合这一精神：先固定结构假设，使后续实证结果能被解释为支持或反对某一原则性架构主张的证据。
...
\textbf{超参数政策}。所有模型将在相同的搜索空间内调优：学习率 \(\{10^{-4}, 3\times10^{-4}, 10^{-3}\}\)，dropout \(\{0.1,0.2,0.3\}\)，匹配的隐藏宽度档次，以及基于验证 AUC 的早停。

\textbf{预算政策}。所有模型将在匹配的有效批量大小、匹配的最大更新次数以及可行的匹配参数档次下进行比较。硬件配置与时钟预算将在实证配套修订版中披露。

\begin{table}[htbp]
\centering
\caption{未来工程修订版基线比较政策}
\label{tab:baselines}
\begin{tabular}{l p{7cm} p{5cm}}
\toprule
\textbf{模型} & \textbf{实现来源} & \textbf{公平性规则} \\
\midrule
...
\begin{itemize}
\item 合法的确切数据预处理细节；
\item 模型超参数、随机种子与硬件配置；
\item 交互块、压缩时机与动态更新的消融表；
\item 参数、数据比例与序列长度上的扩展曲线；
\item 延迟与内存剖面；
\item 允许公开的代码与实验脚本。
\end{itemize}
...
\bibitem{shan2016deep}
Y. Shan, T. R. Hoens, J. Jiao, H. Wang, D. Yu, and J. C. Mao.
\newblock Deep crossing: Web-scale modeling without manually crafted combinatorial features.
\newblock In \emph{Proceedings of the 22nd ACM SIGKDD International Conference on Knowledge Discovery and Data Mining}, pages 255--262, 2016.

\bibitem{cheng2016wide}
H.-T. Cheng, L. Koc, J. Harmsen, et al.
\newblock Wide \& deep learning for recommender systems.
\newblock \emph{arXiv preprint arXiv:1606.07792}, 2016.
...
\newblock In \emph{Proceedings of the 26th International Joint Conference on Artificial Intelligence}, pages 1725--1731, 2017.

\bibitem{wang2017deep}
R. Wang, B. Fu, G. Fu, and M. Wang.
\newblock Deep \& cross network for ad click predictions.
\newblock In \emph{Proceedings of the ADKDD'17}, pages 1--7, 2017.

\bibitem{wang2021dcnv2}
R. Wang, R. Shivanna, D. Cheng, et al.
\newblock DCN V2: Improved deep \& cross network and practical lessons for web-scale learning to rank systems.
\newblock In \emph{Proceedings of the Web Conference 2021}, pages 1785--1797, 2021.
...
\newblock In \emph{Proceedings of the 4th International Conference on Learning Representations}, 2016.

\bibitem{zhou2018deep}
G. Zhou, C. Zhu, R. Song, et al.
\newblock Deep interest network for click-through rate prediction.
\newblock In \emph{Proceedings of the 24th ACM SIGKDD International Conference on Knowledge Discovery and Data Mining}, pages 1059--1068, 2018.

\bibitem{zhou2019deep}
G. Zhou, N. Mou, Y. Fan, et al.
\newblock Deep interest evolution network for click-through rate prediction.
\newblock In \emph{Proceedings of the 33rd AAAI Conference on Artificial Intelligence}, pages 5941--5948, 2019.

\bibitem{kang2018self}
W.-C. Kang and J. McAuley.
\newblock Self-attentive sequential recommendation.
\newblock In \emph{2018 IEEE International Conference on Data Mining}, pages 197--206, 2018.

\bibitem{sun2019bert4rec}
F. Sun, J. Liu, J. Wu, et al.
\newblock BERT4Rec: Sequential recommendation with bidirectional encoder representations from transformer.
\newblock In \emph{Proceedings of the 28th ACM International Conference on Information and Knowledge Management}, pages 1441--1450, 2019.
...
\newblock In \emph{Proceedings of the 24th ACM SIGKDD International Conference on Knowledge Discovery and Data Mining}, pages 1754--1763, 2018.

\bibitem{song2019autoint}
W. Song, C. Shi, Z. Xiao, et al.
\newblock AutoInt: Automatic feature interaction learning via self-attentive neural networks.
\newblock In \emph{Proceedings of the 28th ACM International Conference on Information and Knowledge Management}, pages 1161--1170, 2019.
